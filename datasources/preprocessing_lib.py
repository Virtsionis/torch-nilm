...
def apply_rolling_window(mainchunk, meterchunk, window_size):
...
def create_batches(mainchunk, meterchunk, seq_len):
    ix = mainchunk.index
...
def replace_nans(mainchunk, meterchunk):
    mainchunk.fillna(0, inplace=True)
    meterchunk.fillna(0, inplace=True)
    return mainchunk, meterchunk


def normalize_chunks(mainchunk, meterchunk, mmax):
    if mmax is None:
        mmax = mainchunk.max()
...
def standardize_chunks(mainchunk, meterchunk, mains_mean, mains_std, meter_mean, meter_std):
...
def align_chunks(mainchunk, meterchunk):
...
def replace_with_zero_small_values(mainchunk, meterchunk, threshold):
    mainchunk[mainchunk < threshold] = 0
    meterchunk[meterchunk < threshold] = 0
    return mainchunk, meterchunk


def denoise(mainchunk, meterchunk):
