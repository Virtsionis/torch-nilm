...
from datasources.datasource import Datasource, WaterDatasource
...
class BaseWaterDataset(ABC):
    ...
    """
    def __init__(self, datasource: WaterDatasource, building: int, device: str, start_date: str,
    ...
    def _init_generators(self, datasource: WaterDatasource, sample_period: float, chunksize: int):
        self.datasource = datasource
        self.data_generator = self.datasource.get_data(start=self.start_date,
                                                       end=self.end_date,
                                                       sample_period=self.sample_period,
                                                       chunksize=chunksize)
        self.data_generator.set_index('Time', inplace=True)
        self._reload()

    def _reload(self):
        try:
            mainchunk = self.data_generator['mains']
            meterchunk = self.data_generator[self.device]
...
class WaterDataset(BaseWaterDataset, Dataset):
    """
    -ElectricityDataset
...
class WaterMultiBuildingsDataset(BaseWaterDataset, Dataset):
    """
    ...
    def _init_generators(self, train_info, sample_period, chunksize, **kwargs):
        for (index, element) in enumerate(train_info):
            ...
            self._init_single_building_generators(datasource, device, start_date,
                                                  end_date, sample_period, chunksize, index)

    def _init_single_building_generators(self, datasource: WaterDatasource, device, start_date,
                                         end_date, sample_period, chunksize, index):
        self.datasources[index] = datasource
        self.datas[index] = self.datasources[index].get_data(start=start_date,
                                                             end=end_date,
                                                             sample_period=sample_period,
                                                             chunksize=chunksize)
        self.data_generator = self.datas[index]
        self.data_generator.set_index('Time', inplace=True)
        self._reload(device)
    ...
    def _reload(self, device: str = None):
        try:
            mainchunk = self.data_generator['mains']
            meterchunk = self.data_generator[device]
            mainchunk, meterchunk = align_chunks(mainchunk, meterchunk)
            if len(mainchunk) or len(meterchunk):
                mainchunk, meterchunk = self._chunk_preprocessing(mainchunk, meterchunk)
