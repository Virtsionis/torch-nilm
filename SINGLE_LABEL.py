...
for i, dev in enumerate(devices):
    ...
    model_hparams = [
        {
            ...
            'hparams': {'input_dim': None, 'distribution_dim': 16, 'targets_num': 1,
                        'alpha': 1e-2, 'beta': 1e-3, 'gamma': 1e-1, 'dae_output_dim': 100,
                        'max_noise': 0.1, 'prior_weights': [prior_weights[i]], 'prior_noise': prior_noise,
                        },
        },
        {
    ...
    experiment_parameters = ExperimentParameters(**experiment_parms)

    experiment = NILMSuperExperiments(project_name='SuperVaeSingleLabel', clean_project=False,
                                      devices=[dev], save_timeseries_results=False,
