...
def load_eval(model_name: str, tests_params: pd.DataFrame, sample_period: int,
              batch_size: int, experiment_name: str, iteration: int, device: str, mmax: float,
              means: float, stds: float, meter_means: float, meter_stds: float, window_size: int, root_dir: str,
              model_hparams: dict, eval_params: dict, save_timeseries: bool = True, callbacks=None,
              preprocessing_method: str = SupportedPreprocessingMethods.ROLLING_WINDOW,
              inference_cpu: bool = False, experiment_type: str = None, experiment_category: str = None,
              subseq_window: int = None, saved_models_dir: str = DIR_SAVED_MODELS_NAME,
              output_dir: str = DIR_OUTPUT_NAME, model_file_name: str = None, model_file_path: str = None,
    ...
    if model_file_path and os.path.isfile(model_file_path):
        ...
        if output_dir:
            load_dir = '/'.join([os.getcwd(), output_dir, root_dir])
        else:
            load_dir = '/'.join([os.getcwd(), root_dir])

        model_path = '/'.join([load_dir, experiment_type, saved_models_dir, device, model_name,
                               experiment_category, experiment_name, ''])
        load_path = model_path + ITERATION_NAME + '_' + str(iteration) + '_' + model_file_name + CKPT_EXTENSION
    else:
        raise Exception('Load file was not properly defined. Execution terminated')
