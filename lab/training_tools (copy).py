...
class SuperVariationalTrainingTools(VIBTrainingTools):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('reco_loss', self.alpha*reco_loss, prog_bar=True, on_epoch=True, on_step=False)
        self.log('info_loss', self.beta*info_loss, prog_bar=True, on_epoch=True, on_step=False)
        self.log('class_loss', self.gamma*class_loss, prog_bar=True, on_epoch=True, on_step=False)
    ...
    @staticmethod
    def compute_info_loss(noise_dist, target_dists, noise_info=False, only_noise=False, logvar=False):
        def kl_divergence(sigma, m, logvar=True):
            if logvar:
                return -0.5 * torch.sum(1 + sigma - m.pow(2) - sigma.exp()).mean().div(math.log(2))
            return -0.5 * torch.sum(1 + 2 * sigma.log() - m.pow(2) - sigma.pow(2)).mean().div(math.log(2))

        if only_noise:
            mu, std = noise_dist[0], noise_dist[1]
            return kl_divergence(std, mu, logvar=logvar)

        info_loss = 0
        n = len(target_dists)
        if noise_info:
            mu, std = noise_dist[0], noise_dist[1]
            info_loss += kl_divergence(std, mu, logvar=logvar)
            n += 1
    ...
    @staticmethod
    def compute_class_loss(y, target_logits, reduction='sum'):
        class_loss = 0
        # y must be in shape [[batch_size, 1], [batch_size, 1], ...]
        for i in range(target_logits.shape[-1]):
            logit = target_logits[:, :, :, i].squeeze(1)
            class_loss += F.mse_loss(logit, y[:, i, :], reduction=reduction)
        return class_loss / len(target_logits)

    @staticmethod
    def compute_reconstruction_loss(vae_logit, x, reduction='sum'):
        return F.mse_loss(vae_logit.squeeze(), x.squeeze(), reduction=reduction)
        # return 0
    ...
    def validation_step(self, val_batch: Tensor, batch_idx: int) -> Dict:
        ...
        self.log("val_reco_loss", self.alpha*reco_loss, prog_bar=False, on_epoch=True, on_step=False)
        self.log("val_info_loss", self.beta*info_loss, prog_bar=False, on_epoch=True, on_step=False)
        self.log("val_class_loss", self.gamma*class_loss, prog_bar=False, on_epoch=True, on_step=False)
